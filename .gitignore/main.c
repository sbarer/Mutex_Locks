...
void *thread_0(void *arg) {
    while (lock(&glocks[0]) == 0) { sleep(1); } // Force locking glocks[0]
    printf("\t\tThread 0: received lock 0 -- %ld\n", pthread_self());
    sleep(1);
    while (lock(&glocks[1]) == 0) { sleep(1); } // Force locking glocks[1]
...
void *thread_1(void *arg) {
    while (1) {
        ...
        printf("\t\tThread 1: received lock 1 -- %ld\n", pthread_self());
        sleep(1);
        if (lock1_res) {
